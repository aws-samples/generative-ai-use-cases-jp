...
- `dynamicFilters`: Dynamically creates and applies filters on the application side. (Example: Creating and applying filters based on user attributes such as department) You can also map Cognito Groups or SAML IdP Groups to Attributes for use. (See [SAML Integration with Microsoft Entra ID](./SAML_WITH_ENTRA_ID.md) for details) Additionally, ABAC (Attribute-Based Access Control) filtering using Cognito custom attributes is supported. (See "ABAC Metadata Filter" below)
- `implicitFilters`: If specified, the LLM creates and applies filters for the specified metadata based on the user's question. (Example: Filtering by year mentioned in the user's question to search only data from that year) If an empty array, no filters are applied. Currently only supports Claude Sonnet 3.5. (Throttling may occur due to quotas)
...
#### ABAC (Attribute-Based Access Control) Metadata Filter

You can implement document access control (ABAC) based on user attributes using Cognito User Pool custom attributes. A sample for ABAC is provided in Example 3 of `dynamicFilters`.

**Setup Steps:**

1. **Add a custom attribute to the Cognito User Pool**

   Add a custom attribute (e.g., `custom:department`) to the Cognito User Pool schema. This can be done from the AWS Management Console under Cognito User Pool settings > "Sign-up experience" > "Custom attributes".

2. **Set custom attribute values for users**

   Set the custom attribute value for each user (e.g., `custom:department` = `engineering`).

3. **Configure document metadata**

   Set the corresponding key and value in the metadata.json for documents uploaded to S3.
...
4. **Uncomment `getDynamicFilters` Example 3**

   Uncomment Example 3 in [packages/common/src/custom/rag-knowledge-base.ts](/packages/common/src/custom/rag-knowledge-base.ts) and customize as needed.

> **Note:** Cognito Group-based control (Example 1) and ABAC-based control (Example 3) can be used independently. Choose the one that best fits your use case.

#### S3 Prefix-Based Metadata Filter

You can also control document access using S3 object prefixes (folder structure). Bedrock Knowledge Base automatically generates a metadata attribute called `x-amz-bedrock-kb-source-uri`, which can be used as a filter key. A sample is provided in Example 4 of `dynamicFilters`.

**Prerequisites:**
- OpenSearch Serverless must be used as the vector store (`startsWith` filter is only supported on OpenSearch Serverless)

**Setup Steps:**

1. **Organize documents in S3 by prefix**

   Place documents under prefixes separated by department or team.
...
2. **Set custom attributes for Cognito users**

   Set the attribute corresponding to the allowed prefix for each user (e.g., `custom:department` = `engineering`).

3. **Uncomment `getDynamicFilters` Example 4**

   Uncomment Example 4 in [packages/common/src/custom/rag-knowledge-base.ts](/packages/common/src/custom/rag-knowledge-base.ts) and customize as needed.

> **Note:** This approach does not require adding metadata to metadata.json files. `x-amz-bedrock-kb-source-uri` is a metadata attribute automatically generated by Knowledge Base for filtering operations.
