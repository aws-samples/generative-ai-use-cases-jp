...
このアプリケーションをデプロイするために、AWS CDK を実行する環境が必要です。CDK を使ったデプロイに不慣れな方は、[こちらのドキュメント](https://catalog.workshops.aws/generative-ai-use-cases-jp) をご活用ください。CDK 環境の準備、デプロイの手順の説明、セキュリティ機能の追加などを丁寧に説明したワークショップがあります。
