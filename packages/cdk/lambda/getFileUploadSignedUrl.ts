...
export const handler = async (
  ...
  try {
    ...
    const { tenantAccountId, tenantRegion, tenantEnvironment } =
        ...
        ? await (async () => {
            try {
              ...
              if (!accountId || !tenant.region || !tenant.environment) {
                throw new Error(
                  `Incomplete tenant information for ${tenantId}: accountId=${accountId}, region=${tenant.region}, environment=${tenant.environment}`
                );
              }

              console.log(
                `Tenant info - Account: ${accountId}, Region: ${tenant.region}, Environment: ${tenant.environment}`
              );
              return {
              ...
              console.error(
                `Failed to get tenant info for ${tenantId}:`,
                error
              );
              throw new Error(
                `Cannot generate bucket name without tenant information: ${error}`
              );
            }
          })()
        : {
