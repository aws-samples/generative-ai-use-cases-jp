...
const createBodyText = (
  ...
  return modelConfig.createBodyText(messages, usecase);
};
...
const bedrockApi: ApiInterface = {
  invoke: async (model, messages, usecase?) => {
    const client = await initBedrockClient();
    const command = new InvokeModelCommand({
      modelId: model.modelId,
      body: createBodyText(model.modelId, messages, usecase),
  ...
  invokeStream: async function* (model, messages, usecase?) {
    const client = await initBedrockClient();
    try {
