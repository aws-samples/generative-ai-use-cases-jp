...
import { Model, UnrecordedMessage } from 'generative-ai-use-cases-jp';
...
const endpoint: string = process.env.SAGEMAKER_ENDPOINT || '';
const models: Model[] = JSON.parse(process.env.SAGEMAKER_MODELS || '[]');
...
const createBodyText = (
  ...
  return JSON.stringify({
    inputs: generatePrompt(model, messages),
...
const sagemakerApi: ApiInterface = {
  ...
  invokeStream: async function* (model, messages) {
    ...
    let buffer = '';
    const pt = getPromptTemplate(model.modelName);
