...
async function getTenantLambdaArn(tenantId: string): Promise<string> {
  try {
    ...
    if (lambdaArn) {
      console.log(
        `Found Bedrock Chat Lambda ARN for tenant ${tenantId}: ${lambdaArn}`
      );
      return lambdaArn;
    }
    ...
    console.warn(
      `Bedrock Chat Lambda ARN not found in metadata for tenant ${tenantId}, using fallback pattern`
    );
    ...
    throw new Error(
      `Failed to get Lambda ARN for tenant ${tenantId}: ${error}`
    );
  }
}
 ...
 */
function transformEventForTarget(
...
export const handler = async (
  ...
  try {
    ...
    console.log(
      'Obtained tenant-specific credentials for cross-tenant access prevention'
    );
