...
class ToolManager:
    ...
    def get_tools_with_options(
        ...
        logger.info(
            f"Total tools loaded: {len(all_tools)} "
            f"(MCP: {len(mcp_tools)}, Built-in: 1, "
            f"Code Interpreter: {len(code_interpreter_tools)} - "
            f"{'enabled' if code_execution_enabled else 'disabled'})"
        )
