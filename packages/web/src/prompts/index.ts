...
export const SummarizePrompt = {
  ...
  summaryContext: (sentence: string, context?: string): string => {
    return `以下の <要約対象の文章></要約対象の文章> の xml タグで括られた文章を要約してください。
ただし、出力は、要約した文章だけにしてください。それ以外の文章は一切出力しないでください。もちろん xml のタグの出力もしてはいけません。
...
${
  !context
    ? ''
    : `要約する際、以下の <要約時に考慮して欲しいこと></要約時に考慮して欲しいこと> の xml タグで括られた内容を考慮してください。
<要約時に考慮して欲しいこと>
${context}
</要約時に考慮して欲しいこと>
`
