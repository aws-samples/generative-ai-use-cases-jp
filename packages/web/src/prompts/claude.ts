...
<output></output>`;
  ...
  meetingMinutesPrompt(params: MeetingMinutesParams): string {
    ...
    switch (params.style) {
      ...
      case 'summary':
        return `As a professional meeting assistant, please create a concise summary of the meeting. Include key discussion points, decisions made, and action items. Write in the same language as the received text.`;

      case 'detail':
        return `As a professional meeting assistant, please create detailed meeting minutes. Include all discussion points, participants' opinions, decisions, action items with owners and deadlines. Write in the same language as the received text.`;

      case 'diagram': {
        const diagramTypes = params.diagramOptions?.length
          ? params.diagramOptions.join(', ')
          : 'mindmap';
        return `As a professional meeting assistant, please analyze the meeting content and create ${diagramTypes} diagram(s) using Mermaid syntax to visualize the key concepts, relationships, and flow of the discussion. Write in the same language as the received text.`;
      }

      case 'whiteboard':
        return `As a professional meeting assistant, please create a visual whiteboard-style summary of the meeting. Use bullet points, boxes, and arrows to show relationships between ideas. Write in the same language as the received text.`;
