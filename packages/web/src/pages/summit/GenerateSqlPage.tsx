...
import { GenerateSqlPageQueryParams } from '../../@types/navigate';
...
const GenerateSqlPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (search !== '') {
      const params = queryString.parse(search) as GenerateSqlPageQueryParams;
      setSchemas(params.schemas ?? '');
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [setSchemas, setInstruction, modelId, availableModels, search]);

  useEffect(() => {
    setTypingTextInput(text);
  }, [text, setTypingTextInput]);

  const getGeneratedSql = (schemas: string, instruction: string) => {
    postChat(
      prompter.generateSqlPrompt({
        schemas,
        instruction,
      }),
      true
  ...
  const onClickExec = useCallback(() => {
    if (loading) return;
    getGeneratedSql(schemas, instruction);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [schemas, instruction, loading]);
    ...
    <div className="grid grid-cols-12">
      ...
      <div className="col-span-12 col-start-1 mx-2 lg:col-span-10 lg:col-start-2 xl:col-span-10 xl:col-start-2">
        <Card label="クエリ生成に必要な情報">
          ...
          <Textarea
            placeholder="クエリ生成に必要なテーブル情報 (DDL など) を入力してください"
            value={schemas}
            onChange={setSchemas}
            maxHeight={-1}
          />
